...
`;
    ...
    // Structural Guard: Tells the model precisely what to output if the variable is blank
    userNotesSection = `
USER RAW OBSERVATIONS & SHORTER NOTES
------------------------------------------
No raw notes provided by the user. 

CRITICAL PROCESSING INSTRUCTIONS:
1. Since the user did not provide custom raw notes, you MUST output an empty string "" for the top-level "userNotes" key. Do not make up notes.
`;
  }
...
`.trim(); // FIX: Swapped .strip() for the valid JS string method .trim()
};