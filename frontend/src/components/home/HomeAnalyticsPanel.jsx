      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {(cards.length ? cards : [1, 2, 3, 4]).map((card, idx) => {
          if (typeof card === "number") {
            return (
              <div key={card} className="rounded-2xl border border-border-default bg-bg-surface p-5 shadow-card animate-pulse">
                <div className="h-3 w-24 rounded bg-bg-soft" />
                <div className="mt-4 h-8 w-20 rounded bg-bg-soft" />
          ...
          return (
            <article
              key={idx}
              className="rounded-2xl border border-border-default bg-bg-surface p-5 shadow-card hover:border-border-strong transition-all"
            >
              <div className="flex items-center justify-between">
                <p className="text-[11px] uppercase tracking-[0.18em] text-text-muted font-semibold">
                  {card.label}
                </p>
                <div className="h-8 w-8 rounded-lg border border-primary/20 bg-primary/10 text-primary inline-flex items-center justify-center">
              ...
              <p className="mt-2 text-xs text-text-light leading-5">
                {card.subtitle}
              </p>
            </article>
          );
