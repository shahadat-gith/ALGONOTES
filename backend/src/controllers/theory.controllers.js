...
export const generateAiTheory = async (req, res, next) => {
  try {
    ...
    try {
      ...
      await Theory.findByIdAndDelete(theoryIdStr);
...
export const deleteTheoryImage = async (req, res, next) => {
  try {
    ...
    const publicId = remainder.replace(/\.[^/.]+$/, ""); // Strip file extension out cleanly
...
export const deleteTheory = async (req, res, next) => {
  try {
    ...
    // Cascading assets cleanup directly using Admin API calls
    cloudinary.api.delete_resources_by_prefix(folderPath)
      .then(() => cloudinary.api.delete_folder(folderPath))
      .catch((err) => console.warn(`[Cloudinary Cleanup Exception Warning]: ${err.message}`));

    await Theory.findByIdAndDelete(theory_id);
